...
services_trade <- read_xls("/tmp/tradeInServices_export.xls", skip = 2) %>% data.table(check.names = TRUE)

services_trade %<>% .[X__3 != "<NA>"] %>% .[,.SD, .SDcols = ! c("X__1","X__2")]

setnames(services_trade, "X__3", "country")

services_trade %<>% .[,lapply(.SD,as.numeric), by = country]

services_exports <-
services_trade[,.(exportsServiecs2017 =sum(c(X2017.Q1,X2017.Q2,X2017.Q3,X2017.Q4), na.rm = TRUE)), by = country]
...
GDPbyCountry <- fread("http://databank.worldbank.org/data/download/GDP.csv", skip = 3, check.names = TRUE)
